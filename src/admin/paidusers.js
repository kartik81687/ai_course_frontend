...
import { FaUserCircle, FaEnvelope, FaCalendar, FaCreditCard, FaTag } from 'react-icons/fa';
    ...
    const fetchPaidUsers = async () => {
        try {
            ...
            const transformedUsers = response.data.map(user => ({
                ...user,
                planType: user.planType || 'N/A',
                planPrice: user.planPrice || 'N/A',
            ...
            showToast('Failed to fetch paid users');
            setLoading(false);
        }
    };
            ...
            <div className="md:hidden relative z-10">
                <div className="bg-luxury/50 backdrop-blur-md border-b border-white/10">
                    <div className="flex items-center justify-between px-4 py-4">
                        <h1 className="text-[#FFD700] text-xl font-black">Paid Users</h1>
                        <button 
                                        ...
                                        <table className="w-full">
                                            <thead>
                                                <tr className="bg-white/5">
                                                    ...
                                                    <th className="px-6 py-4 text-left text-sm font-semibold text-[#FFD700]">Plan</th>
                                                    <th className="px-6 py-4 text-left text-sm font-semibold text-[#FFD700]">Amount</th>
                                            ...
                                            <tbody className="divide-y divide-white/10">
                                                {currentUsers.map((user) => (
                                                    <tr key={user._id} className="hover:bg-white/5 transition-colors">
                                                        ...
                                                        <td className="px-6 py-4">
                                                            <div className="flex items-center space-x-2">
                                                                <FaTag className="text-[#FFD700]" />
                                                                <span className="text-white/70">{user.planType}</span>
                                                            </div>
                                                        </td>
                                                        <td className="px-6 py-4">
                                                            <div className="flex items-center space-x-2">
                                                                <FaCreditCard className="text-[#3498DB]" />
                                                                <span className="text-white/70">${user.planPrice}</span>
                                                            </div>
                                                        </td>
                                                        <td className="px-6 py-4">
                                                            <div className="flex items-center space-x-2">
